    ...
    def train(self, train_data, train_label):
        with tf.Session() as sess:
            ...
            for epoch in range(train_epochs):
                ...
                if (min_loss > loss):
                    min_loss = loss
                    saver.save(sess, './model_tr_3in1/weights_final.ckpt', global_step = epoch + 1)

    def keeptrain(self, train_data, train_label):

    	saver = tf.train.import_meta_graph('./model_tr_3in1/weights_final.ckpt-99806.meta')
        with tf.Session() as sess:
            saver.restore(sess,'./model_tr_3in1/weights_final.ckpt-99806')
            with tf.name_scope('Cost'):
                cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.out, labels = self.y)) + self.lossL2
                tf.summary.scalar(name = 'cost', tensor = cost)
                update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
...
def main():
    train_csv = pd.read_csv('merge1_2m_3in1.csv')
