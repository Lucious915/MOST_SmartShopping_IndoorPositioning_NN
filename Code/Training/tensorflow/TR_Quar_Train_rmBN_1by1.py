...
class NetWork():
    ...
    def creat_network(self):
        with tf.name_scope('Input_layer'):
            self.x = tf.placeholder(tf.float32, [None, 12])
            self.y = tf.placeholder(tf.float32, [None, 32])
            self.train_flag = tf.placeholder(tf.bool)
            self.keep_prob = tf.placeholder(tf.float32)

        x_ = tf.reshape(self.x, shape = [-1, 3, 4, 1])
        x = (x_ - 60)/40
        conv = self.__conv_2d('Conv_layer', x, 3, 100)
        fc_flat = self.__fc_flatten('Flat_layer', conv)
        fc = self.__fc_dense('FC_layer', fc_flat, 32)
        # batch = self.__batch_norm('Batch_norm', fc, self.train_flag)
        drop = self.__drop_out('Drop_layer', fc, self.keep_prob)
        self.out = self.__fc_dense('Output_layer', drop, 32)
        self.lossL2 = tf.add_n([tf.nn.l2_loss(var) for var in tf.trainable_variables() if 'bias' not in var.name]) * 0.001
        with tf.Session() as sess:
            tf.train.write_graph(sess.graph, './model_tr_1by1', 'graph.pbtxt')
    ...
    def train(self, train_data, train_label):
        with tf.Session() as sess:
            ...
            writter = tf.summary.FileWriter('./logs_tr_1by1/train', sess.graph)
            ...
            for epoch in range(train_epochs):
                ...
                if (min_loss > loss):
                    min_loss = loss
                    saver.save(sess, './model_tr_1by1/weights_final.ckpt', global_step = epoch + 1)

    def keeptrain(self, train_data, train_label):

    	saver = tf.train.import_meta_graph('./model_tr_3in1/weights_final.ckpt-99806.meta')
        with tf.Session() as sess:
            saver.restore(sess,'./model_tr_1by1/weights_final.ckpt-99806')
            with tf.name_scope('Cost'):
                cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.out, labels = self.y)) + self.lossL2
                tf.summary.scalar(name = 'cost', tensor = cost)
                update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            ...
            writter = tf.summary.FileWriter('./logs_tr_3in1/train', sess.graph)
            merged = tf.summary.merge_all()
            # tf.global_variables_initializer().run()
            # saver = tf.train.Saver(max_to_keep = 1)
...
def main():
    train_csv = pd.read_csv('merge1_2m_1by1.csv')
    train = np.array(train_csv)
    train_data = -np.hsplit(train, (1, 13))[1]
    print train_data
    ...
    # network.keeptrain(train_data, train_label_OneHot)
