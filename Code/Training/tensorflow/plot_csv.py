...
class NetWork():
    ...
    def creat_network(self):
        ...
        with tf.Session() as sess:
            tf.train.write_graph(sess.graph, './model2', 'graph.pbtxt')
    ...
    def train(self, train_data, train_label):
        with tf.Session() as sess:
            ...
            writter = tf.summary.FileWriter('./logs2/train', sess.graph)
            ...
            for epoch in range(train_epochs):
                ...
                if (min_loss > loss):
                    min_loss = loss
                    saver.save(sess, './model2/weights_final.ckpt', global_step = epoch + 1)

def main():
    train_csv = pd.read_csv('merge.csv')

    train = np.array(train_csv)
    train_data = -np.hsplit(train, (0, 12))[1]
    train_data_df = pd.DataFrame(train_data)
    df=train_data_df.unstack().reset_index()
    df.columns=["X","Y","Z"]
    ...
    # sns.pairplot(train_data_pd)



    # train_label = np.hsplit(train, (0, 27))[2]
    # print train_label
