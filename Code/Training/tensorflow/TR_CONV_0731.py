...
class NetWork():
    ...
    def creat_network(self):
        with tf.name_scope('Input_layer'):
            ...
            self.y = tf.placeholder(tf.float32, [None, 16])
        ...
        self.out = self.__fc_dense('Output_layer', fc_flat, 16)
    ...
    def train(self, train_data, train_label):
        with tf.Session() as sess:
            ...
            writter = tf.summary.FileWriter('./logs_tr_conv_0731/train', sess.graph)
            ...
            for epoch in range(train_epochs):
                ...
                if (min_loss > loss):
                    min_loss = loss
                    saver.save(sess, './model_tr_conv_0731/weights_final.ckpt', global_step = epoch + 1)

def main():
    train_csv = pd.read_csv('20190731TR.csv')
    train = np.array(train_csv)
    train_data = np.hsplit(train, (0, 13))[1]
    print train_data
    train_label = np.hsplit(train, (0, 13))[2]
    print train_label
