...
threshold = 20
saver = tf.train.import_meta_graph('/media/lucious915/Data/05_Laboratory/DaTung_Indoor_Positioning/CYCODE/Code/Training/tensorflow/model/weights_final.ckpt.meta')

def predict(rssi_data_x):
    with tf.Session() as sess:
        saver.restore(sess, '/media/lucious915/Data/05_Laboratory/DaTung_Indoor_Positioning/CYCODE/Code/Training/tensorflow/model/weights_final.ckpt')
...
def main():
    ...
    if ((last_local == -1) or (difference >= threshold)):
        ...
        if ((current_local % 3) == (last_local % 3)):
            print(current_local)
        else:
            print(last_local)
